...
void gotoTime(uint8_t h, uint8_t m) {
  ...
  // Compute absolute targets via shortest path
  long absMinTarget = shortestPathTarget(stepMin.currentPosition(), minPhase, USTEPS_MIN);
  long absHrTarget  = shortestPathTarget(stepHr .currentPosition(), hrPhase,  USTEPS_HR);
...
void handleCue(const Cue& c) {
  switch (c.type) {
    ...
    case CUE_RUN:
      ...
      carryMin = 0;
      carryHr  = 0;
      lastMs   = millis();     // ensure first dt is small
      break;
...
void setup() {
  Serial.begin(115200);
  delay(200);
  Serial.println(F("[SETUP] Stage clock (no RTC, µstep=1/4, variable nudges)"));
...
void loop() {
  ...
  if (mode == MODE_CUE) {
    if (stepMin.distanceToGo() || stepHr.distanceToGo()) {
      ...
      carryMin = 0;
      carryHr  = 0;
      lastMs   = millis();   // start RUN timing cleanly
    }
    return;
  }
  ...
  if (mode == MODE_RUN && running) {
    ...
    // Minute shaft: 1 rev / 60 minutes
    float uMin = (float)USTEPS_MIN / (60.0f * 60000.0f);
    float iMin = dt * uMin + carryMin;
    long  wMin = (long)iMin;
    carryMin   = iMin - wMin;
    if (wMin) stepMin.move(wMin);

    // Hour shaft: 1 rev / 12 hours = 720 minutes
    float uHr = (float)USTEPS_HR / (720.0f * 60000.0f);
